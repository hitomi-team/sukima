...
class GPTHF(GPTAuto):
    ...
    @torch.inference_mode()
    def classify(self, args, *, db_softprompt: Optional[SoftPromptModel] = None):
        logits_processors = []
        stopping_criterion = []
        softprompt = None
        ...
        if softprompt:
            prompt_length += 20
        stopping_criterion.append(MaxLengthCriteria(prompt_length+1))

        logits_processor = LogitsProcessorList(logits_processors)
        stopping_criterion = StoppingCriteriaList(stopping_criterion)
        ...
        scores = torch.nn.functional.softmax(output.scores[0], dim=-1)
        output_logits = {}
        for label in args["labels"]:
