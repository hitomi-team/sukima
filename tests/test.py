...
def test_login():
    response = client.post("/api/v1/users/token", json={"username": "test", "password": "test"})
...
def test_login_load_infer():
    login_response = client.post("/api/v1/users/token", json={"username": "test", "password": "test"})

    assert login_response.status_code == 200
    token = login_response.json()["access_token"]

    test_model = "distilgpt2"
    load_model_response = client.post(
        "/api/v1/models/load",
    ...
    infer_model_response = client.post(
